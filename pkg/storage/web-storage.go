...
func (s *WebStorage) CreateUser(username, passwordHash string) (*models.User, error) {
...
func (s *WebStorage) GetUserByUsername(username string) (*models.User, error) {
...
func (s *WebStorage) GetUserByID(id int) (*models.User, error) {
	var user models.User
...
func (s *WebStorage) CreateTrade(trade *models.Trade) (int, error) {
	result, err := s.db.Exec(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, trade.UserID, trade.MasterAccountID, trade.Symbol, trade.Side, trade.Volume, trade.Leverage, trade.SentAt, trade.Status)
	if err != nil {
		return 0, err
	}
...
func (s *WebStorage) GetTrades(userID int, limit, offset int) ([]models.Trade, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&trade.SentAt, &trade.ReceivedAt, &trade.ExchangeAcceptedAt,
